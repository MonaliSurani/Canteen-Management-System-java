...
export function EmployeeEditForm({ member, closeModal }) {
    const [formData, setFormData] = useState({
        contact: '',
        password: '',
        email: '' // Assuming the unique identifier for an employee is their email
    });
    ...
    useEffect(() => {
        // Populate form with current employee's data
        if (member) {
            setFormData({
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            const result = await axios.post(`${BASE_URL}/employee/update`, formData);
            console.log(result);
            setIsSubmitted(true);
    ...
    return (
        <Container>
            ...
            {isLoading ? (
                ...
                <Alert variant="success">Employee updated successfully</Alert>
            ) : (
